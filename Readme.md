...
| `Stocks` | Stock price index (weighted avg: 0.5 S&P 500 + 0.25 NASDAQ Composite + 0.25 NASDAQ 100) | S&P Dow Jones / NASDAQ OMX (FRED series: NASDAQCOM, NASDAQ100) | Log-difference × 100 |
| `Spread` | Corporate bond spread (average of AAA and BAA, relative to 10Y Treasury) | Moody's (FRED series: DAAA, DBAAA) | First difference |
...
**Notes:** Holidays and weekends are excluded. Remaining missing values are linearly interpolated up to 4 working days. Transformations are applied after treatment of missing values.
...
| `SLSAP` | Large-scale asset purchase surprise | Swanson (2021) |
...
| 2020–2022 | Board of Governors (https://federalreserve.gov/monetarypolicy/fomccalendars.htm) |

All other weekdays, excluding holidays, are used as control days.
...
The script is written for GNU Octave (tested with Octave 8.x) with the `optim` and `statistics` packages. It is also compatible with Matlab with the Optimization Toolbox.
...
- Burri, M. and Kaufmann, D. (2024). Measuring monetary policy shocks. IRENE Working Paper 24-03, University of Neuchâtel, https://ideas.repec.org/p/irn/wpaper/24-03.html.
