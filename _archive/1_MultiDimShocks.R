...
cumRep   <- rep(TRUE, N)        # Cumulative or non-comulative IRFs (not important because only H = 1)
...
for(firstShock in c("FFR", "IR3Mfed", "IR6Mfed", "IRSTfed")){
  ...
  for(secondShock in c("IR2Yfed", "IR3Yfed", "IR5Yfed", "IRMTfed")){
    ...
    resLP   <- estimLPHet(y[, 1:N], O, Ind, Interact = FALSE, E , P , cumRep, H = 1, HStep, NormFac)
    resHF   <- estimLPProxy(y[, 1:N], O, Z, recursive = FALSE, Ind, E , P , cumRep, H = 1, HStep, NormFac)
...
N        <- length(irfVars)     
cumRep   <- rep(TRUE, N)                      # Cumulative  IRFs
...
# Estimate the models
resHET     <- estimLPHet(y, O, Ind, Interact = FALSE, E , P , cumRep, H, HStep, NormFac)
resHF      <- estimLPProxy(y, O, Z, recursive = FALSE, Ind, E , P , cumRep, H, HStep, NormFac)
resHFrec   <- estimLPProxy(y, O, Z, recursive = TRUE, Ind, E , P , cumRep, H, HStep, NormFac)

# Plot baseline IRFs (HET-IV)
myGraphs <- plotIRFs(NA, resHET$irf, resHET$se, HTick, varLabs[irfVars], shockLabs)
for(e in 1:E){
...
myGraphs <- plot2IRFs(resHET$irf, resHET$se, resHF$irf, resHF$se, HTick, varLabs[irfVars], shockLabs)
for(e in 1:E){
...
myGraphs <- plot2IRFs(resHET$irf, resHET$se, resHFrec$irf, resHFrec$se, HTick, varLabs[irfVars], shockLabs)
for(e in 1:E){
...
HETShocks <- extractShocksKF(resHET$Sig, resHET$SigR, as.matrix(resHET$Psi), as.matrix(resHET$et), tol = tolInv)
HETShocks <- xts(HETShocks, order.by = Dt)
colnames(HETShocks) <- c("Target (HET-IV)", "Path (HET-IV)")

HFShocks <- extractShocksKF(resHF$Sig, resHF$SigR, as.matrix(resHF$Psi), as.matrix(resHF$et), tol = tolInv)
HFShocks <- xts(HFShocks, order.by = Dt)
colnames(HFShocks) <- c("Target (HF-IV)", "Path (HF-IV)")

HFrecShocks <- extractShocksKF(resHFrec$Sig, resHFrec$SigR, as.matrix(resHFrec$Psi), as.matrix(resHFrec$et), tol = tolInv)
...
if(bootstrap == TRUE){
  ...
  resHET_o     <- estimLPHetFast(y, O, Ind, Interact = FALSE, E , P , rep(T, N), H, HStep, NormFac)
  resHF_o      <- estimLPProxyFast(y, O, Z, recursive = F, Ind, E , P , rep(T, N), H, HStep, NormFac)
  ...
  for(b in 1:B){
    ...
    resHET_b     <- estimLPHetFast(y_b, O_b, Ind_b, Interact = FALSE, E , P , rep(T, N), H, HStep, NormFac)
    resHF_b      <- estimLPProxyFast(y_b, O_b, Z_b, recursive = F, Ind, E , P , rep(T, N), H, HStep, NormFac)
  ...
  myGraphs <- plotPvals(p_boot, HTick, varLabs[irfVars], shockLabs)
  for(e in 1:E){
